...
import * as mflTypes from '../../providers/mfl/mfl.service';
import * as fantasyProsTypes from '../../providers/fantasy-pros/fantasy-pros.service';
...
export class PlayersManagerService {
    ...
    async updateRankings(force: boolean = false): Promise<void> {
        try {
            this.logger.log('Updating Fantasy Pros rankings in db.');
            ...
            if (getNewPlayers || force) {
                ...
                if (mongoRankings) {
                    this.logger.log(`Replaced rankings in db.`);
                    ...
                    this.logger.log(`Inserted rankings in db.`);
                    const result = await this.collectionRankings.insertOne(fpRankings);
                    this.rankingsSubject.next(result.ops[0]);
                }

                this.logger.log('Updating MFL players with rankings in db.');
                ...
                if (playersDocs) {
                    ...
                    for (const rankingsPosition of POSITIONS) {
                        this.logger.log(`Updating rankings for position: ${rankingsPosition}`);
                            ...
                            const player = players.player.find(thisPlayer => {
                                ...
                                if (!nameMatch1) {
                                    ...
                                    if (ratio < 90) {
                                        ...
                                        this.logger.log(`Fuzzy matched '${playerName}' and '${rankingName}'`);
                                    }
                                }
                    ...
                    this.logger.log(`Updated players in db with rankings.`);
    ...
    async updateByeWeeks(players: Players): Promise<void> {
        ...
        if (byeWeeksByTeam) {
            ...
            this.logger.log('Updated player bye weeks.');
        }
    }
    ...
    async updateTeamSchedules(players: Players): Promise<void> {
        ...
        if (teamSchedulesByTeam) {
            ...
            this.logger.log('Updated player schedules.');
        }
    }
    ...
    async updateInjuries(players: Players): Promise<void> {
        ...
        if (injuriesByPlayer) {
            ...
            this.logger.log('Updated player injuries.');
        }
    }

    async update(): Promise<void> {
        try {
            this.logger.log('Updating MFL players in db.');
            ...
            if (getNewPlayers) {
                ...
                if (mflPlayers) {
                    if (since) {
                        ...
                        this.logger.log(`Updated ${mflPlayers.player.length} players in db since ${since}.`);
                        ...
                        this.logger.log(`Inserted ${mflPlayers.player.length} players in db.`);
                        const result = await this.collectionPlayers.insertOne(mflPlayers);
                        this.playersSubject.next(result.ops[0]);
                        forceUpdateRankings = true;
                    }
                } else if (players) {
                    this.logger.log('Unable to update players, returning current players.');
                    this.playersSubject.next(players as Players);
                }
            } else {
                this.logger.log('Returning current players.');
                this.playersSubject.next(players as Players);
            }
